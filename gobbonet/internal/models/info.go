...
// Installed returns the models present in the model directory, using the same
// cached scan that backs /models-list.json.
//
// Added for the settings panel's add-a-model modal, which needs to mark
// catalogue entries the user already has. It shares scanCached rather than
// listing the directory again so both answers come from one cache and cannot
// disagree about what is installed.
func (i *Info) Installed() []Record { return i.scanCached() }
