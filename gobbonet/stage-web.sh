...
want_js=$(/usr/bin/grep -c 'src="js/' chat.html)
want_css=$(/usr/bin/grep -c 'href="css/' chat.html)
have_js=$(/usr/bin/find js -maxdepth 1 -name '*.js' | /usr/bin/wc -l)
have_css=$(/usr/bin/find css -maxdepth 1 -name '*.css' | /usr/bin/wc -l)
...
TMP="$(/usr/bin/mktemp -d "${OUT}.staging.XXXXXX")"
trap '/bin/rm -rf "$TMP"' EXIT

for f in $REQUIRED_FILES; do /bin/cp "$f" "$TMP/$f"; done
for d in $REQUIRED_DIRS; do /bin/cp -r "$d" "$TMP/$d"; done
...
/bin/cp gobbonet.ico "$TMP/favicon.ico"
...
fonts=""
if [ -d fonts ]; then
    /bin/cp -r fonts "$TMP/fonts"
    fonts=" + $(/usr/bin/find fonts -maxdepth 1 -type f | /usr/bin/wc -l) font(s)"
fi
